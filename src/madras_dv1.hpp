...
class ptr_group_map {
  ...
  public:
    ...
    __fq1 __fq2 void init_ptr_grp_map(inner_trie_fwd *_dict_obj, uint8_t *_trie_loc, uint64_t *_bm_loc, uint8_t _multiplier,
      ...
      uint32_t idx2_ptr_count = cmn::read_uint32(data_loc + 16);
      ...
      idx2_ptrs_map_loc = data_loc + cmn::read_uint32(data_loc + 20);

      uint8_t *ptrs_loc = data_loc + cmn::read_uint32(data_loc + 24);
    ...
    __fq1 __fq2 void load_inner_trie(uint8_t *trie_bytes) {
      ...
      if (key_count > 0) {
        ...
        if (encoding_type == 't' || *tail_data_loc == 1) {
          ...
          tail_grp_map->init_ptr_grp_map(this, trie_loc, trie_level == 0 ? tf_loc : tf_ptr_loc, trie_level == 0 ? 4 : 1, tails_loc, key_count, node_count, true);
  ...
  public:
    ...
    __fq1 __fq2 uint8_t *get_words_loc(uint32_t node_id, uint32_t *p_ptr_byt_count = nullptr) {
      ...
      if (*p_ptr_byt_count == UINT32_MAX)
        *p_ptr_byt_count = ptr_reader.get_ptr_byts_words(node_id);
    ...
    __fq1 __fq2 void get_val_str(gen::byte_str& ret, uint32_t val_ptr, uint8_t grp_no, size_t max_valset_len) {
      ...
      if (*t < 32) {
        uint32_t bin_len = tail_ptr_flat_map::read_len(t);
        while (*t < 32)
          t++;
        while (bin_len--)
          ret.append(*t++);
