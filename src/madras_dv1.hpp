...
class bv_lookup_tbl {
  ...
  public:
    ...
    uint32_t block_select1(uint32_t target_count, uint32_t& bv_pos) {
      ...
      int pos3;
      for (pos3 = 0; pos3 < width_of_bv_block_n && bv_pos + nodes_per_bv_block_n < node_count; pos3++) {
    ...
    uint32_t select1(uint32_t target_count) {
      if (target_count == 0)
        return 0;
      uint32_t bv_pos = 0;
      uint32_t block_count = block_select1(target_count, bv_pos);
    ...
    void init(uint8_t *_lt_rank_loc, uint8_t *_lt_sel_loc, uint32_t _node_count, uint64_t *_bm_start_loc) {
    ...
    void load_inner_trie() {
      ...
      if (key_count > 0) {
        ...
        if (tail_lt_loc != nullptr) {
          ...
          tail_lt->init(tail_lt_loc, nullptr, node_count, tf_ptr_loc);
        }
      }
...
class val_ptr_group_map : public ptr_group_map {
  ...
  public:
    ...
    void get_delta_val(uint32_t node_id, size_t *in_size_out_value_len, void *ret_val) {
      ...
      uint64_t bm_leaf = UINT64_MAX;
      if (key_count > 0)
        bm_leaf = tf.flags_loc[node_id / nodes_per_bv_block_n];
      int64_t col_val = 0;
      do {
        if (bm_mask & bm_leaf) {
    ...
    void load_into_vars() {
      ...
      if (val_count > 0) {
        ...
        for (uint32_t i = 0; i < val_count; i++) {
          ...
          val_map[i].init(this, trie_loc, tf_leaf.flags_loc, val_buf, key_count, node_count, false);
        }
      }
    }
};
